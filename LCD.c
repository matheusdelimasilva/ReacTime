...
*/
void LCD_DisplayTime(uint8_t* time) {
	LCD_WriteChar(time, 0, 0, 0);
	time++;
	LCD_WriteChar(time, 1, 0, 1);
	time++;
	LCD_WriteChar(time, 0, 0, 2);
	time++;
	LCD_WriteChar(time, 0, 1, 3);
	time++;
	LCD_WriteChar(time, 0, 0, 4);
	time++;
	LCD_WriteChar(time, 0, 0, 5);
	time++;
...
void LCD_PIN_Init(void){
	...
	// Port B
	//set mode register to 10
	// Configure Port B pins as Alternate Function Mode 
	GPIOA->MODER &= ~(0xFF0C0F0F);
	GPIOA->MODER |= 0xA080A0A;
	
	// Configure Port B as Alternate Function 11
	//0,1,4,5,9,12,13,14,15
	GPIOA->AFR[0] &= ~(0x00FF00FF);
	GPIOA->AFR[0] |= 0x00BB00BB;
	GPIOA->AFR[1] &= ~(0xFFFF00F0);
	GPIOA->AFR[1] |= 0xBBBB00B0;
	
	// Port C
	//set mode register to 10
	// Configure Port B pins as Alternate Function Mode 
	GPIOA->MODER &= ~(0x0003FFC0);
	GPIOA->MODER |= 0x0002AA80;
	
	// Configure Port C as Alternate Function 11
	// pins 3,4,5,6,7,8
	GPIOA->AFR[0] &= ~(0xFFFFF00); //was here
	GPIOA->AFR[0] |= 0xBBBBB00;
	GPIOA->AFR[1] &= ~(0x0000000F);
	GPIOA->AFR[1] |= 0x0000000B;
	
	// Port D
	//set mode register to 10
	// Configure Port d pins as Alternate Function Mode 
	GPIOA->MODER &= ~(0xFFFF0000);
	GPIOA->MODER |= 0xAAAA0000;
	
	// Configure Port d as Alternate Function 11
	// 8,9,10,11,12,13,14,15
	GPIOA->AFR[0] &= ~(0x0000000);
	GPIOA->AFR[0] |= 0x00000000;
	GPIOA->AFR[1] &= ~(0xFFFFFFFF);
	GPIOA->AFR[1] |= 0xBBBBBBBB;
}
...
void LCD_Configure(void){
	...
	//Configure DUTY[2:0] bits of LCD_CR and set the duty to 1/4 (011)	
	//BIT 2 =0, BIT1 = 1, BIT0, = 1
	LCD->CR &= ~(LCD_CR_DUTY);
	LCD->CR |= LCD_CR_BIAS_1; //Toggling bit1
	LCD->CR |= LCD_CR_BIAS_0; //Toggling bit0
	
	//Configure CC[2:0] bits of LCD_FCR and set the 
	//contrast value to the maximum 111
	LCD->FCR &= ~(LCD_FCR_CC);
	LCD->FCR |= LCD_FCR_CC_2; //Toggling bit2
	LCD->FCR |= LCD_FCR_CC_1; //Toggling bit1
	LCD->FCR |= LCD_FCR_CC_0; //Toggling bit0
	
	//Configure PON[2:0] bits of LCD_FCR and set the 
	//pulse on period to the maximum (111)
	LCD->FCR &= ~(LCD_FCR_PON);
	LCD->FCR |= LCD_FCR_CC_2; //Toggling bit2
	LCD->FCR |= LCD_FCR_CC_1; //Toggling bit1
	LCD->FCR |= LCD_FCR_CC_0; //Toggling bit0
	...
	LCD->CR &= ~(LCD_CR_VSEL); //LCD_CR_VSEL 
